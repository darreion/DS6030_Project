...
```
```{r}
haiti_elastic_best <- select_best(haiti_elastic_tune, "roc_auc")
```
```{r}
...
haiti_elastic_predictions <- collect_predictions(haiti_elastic_cv)
```
```{r}
...
elastic_auc <- roc_auc(haiti_augment_elastic, BT_class, .pred_TRUE, event_level="second")
svm_lin_auc
...
```
```{r}
svm_elastic_augment_test <- augment(haiti_elastic_final, new_data=Haiti_holdout)
```
