...
```{r}
haiti_augment <- augment(haiti_LDA_fit_cv, new_data=haiti)

performance_lda <- probably::threshold_perf(lda_data, Hazardous, .pred_TRUE, 
...
performance_haiti <- probably::threshold_perf(haiti_augment, Class, .predBlueTarp , 
                                        seq(0.00, 1.0, 0.1), event_level="second")
metrics_haiti <- pivot_wider(performance_haiti, id_cols=.threshold, names_from=.metric,
                       values_from=.estimate)
roc_haiti <- roc_curve(haiti_augment, Class, .predBlueTarp, event_level="second") %>% 
    autoplot() +
